...
@stop
@section('content')
<form id="ajax" action="{{ action('Order\OrderController@update', ['id' => $order->id]) }}" method="POST">
    @csrf
    @method('PATCH')
    <x-flash />
    <label>
    ...
    <label>
        ...
        @error('delegate_id')<span class="text-red-600 font-bold"> {{ $message }}</span>@enderror
    </label>
    <label>
        <span class="label-txt">Shop Name</span>
        <input type="text" name="shop_name" value="{{ $order->shop_name ?? old('shop_name') }}"
    ...
    <label>
        ...
        <input type="text" name="customer_address" value="{{ $order->customer_address ?? old('customer_address') }}"
    ...
    <label>
        ...
        <input type="text" name="phone" value="{{ $order->phone ?? old('phone') }}" placeholder="Enter Phone Number"
            class="input" autocomplete="off">
        <div class="line-box">
            <div class="line"></div>
        </div>
        @error('phone')<span class="text-red-600 font-bold float-left">{{ $message }}</span>@enderror
    </label>
    <label>
        <div class="input-group">
        ...
        @error('order_fees')<span class="text-red-600 font-bold"> {{ $message }}</span>@enderror
    </label>
    <label>
        <span class="label-txt">Delivery Value</span>
        <input type="text" name="delivery_value" value="{{ $order->delivery_value ?? old('delivery_value') }}"
            class="input" autocomplete="off">
        <div class="line-box">
            <div class="line"></div>
        </div>
    ...
    <label>
        ...
        <input type="date" value="{{ $order->delivery_date ?? old('delivery_date') }}" name="delivery_date"
    ...
    <input type="button" id="edit-ajax" value="Edit Ajax Data" class="ml-1 btn btn-success">
</form>
@endsection