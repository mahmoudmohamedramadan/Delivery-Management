    ...
    <tbody>
        @foreach ($delegateWithOrders as $delegate_orders)
          <tr id="row_id">
                ...
                <td><a href="/index/delegate/{{ $delegate_orders->id }}">{{ $delegate_orders->full_name }}</a></td>
                <td>{{ $delegate_orders->national_id }}</td>
                <td>{{ $delegate_orders->phone_number }}</td>
                @if ($delegate_orders->delegate_image)
                <td><img src="{{ asset('images/delegate/'.$delegate_orders->delegate_image) }}" alt="{{ $delegate_orders->delegate_image }}" width="80px" height="80px"></td>
